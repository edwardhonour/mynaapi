...
//	$output['user']=$user;
	...
	$sql="select count(*) as c from nua_employee where company_id in (select id from nua_company where status in ('Enrolling','Accepted'))";
    $quotes=$this->X->sql($sql);
	...
	$sql="select count(*) as c from nua_census";
        $quotes=$this->X->sql($sql);
	...
	foreach($orgs as $z) {
		$rr=array();
		$rr['id']=$z['id'];
		$rr['create_timestamp']=$z['create_timestamp'];
		$rr['org_name']=$z['org_name'];
		$rr['status']=$z['status'];
		$rr['type']=$z['type'];
	    $sql="select count(*) as c from nua_broker where org_id = " . $z['id'];
		$qu=$this->X->sql($sql);
        $rr['broker_count']=$qu[0]['c'];

	    $sql="select count(*) as c from nua_broker where org_id = " . $z['id'];
		$qu=$this->X->sql($sql);
        $rr['broker_count']=$qu[0]['c'];

	    $sql="select count(*) as c from nua_company where org_id = " . $z['id'];
		$qu=$this->X->sql($sql);
        $rr['prospect_count']=$qu[0]['c'];		

	    $sql="select count(*) as c from nua_company where org_id = " . $z['id'] . " and has_accepted_quote = 'Y'";
		$qu=$this->X->sql($sql);
        $rr['accepted_count']=$qu[0]['c'];		
		
	    $sql="select count(*) as c from nua_employee_enrollment where company_id in (select id from nua_company where org_id = " . $z['id'] . ")";
		$qu=$this->X->sql($sql);
        $rr['enrollment_count']=$qu[0]['c'];		
		
		array_push($q,$rr);
	}
	$output['orgs']=$q;

    $sql="select * from nua_quote where status in ('Submitted','Accepted','Enrolling') order by id";
    $quotes=$this->X->sql($sql);	
	$q=array();
	foreach($quotes as $z) {
		$rr=array();
		$rr['id']=$z['id'];
		$rr['create_timestamp']=$z['create_timestamp'];
		$rr['quote_name']=$z['quote_name'];
		$rr['status']=$z['status'];
	        $sql="select org_name from nua_org where id = " . $z['org_id'];
		$qu=$this->X->sql($sql);
                if (sizeof($qu)>0) {
	            $rr['org_name']=$qu[0]['org_name'];
		} else {
		    $rr['org_name']="BAD DATA";
		}
	        $sql="select company_name from nua_company where id = " . $z['company_id'];
		$qu=$this->X->sql($sql);
                if (sizeof($qu)>0) {
                       $rr['company_name']=$qu[0]['company_name'];
		} else {
                        $rr['company_name']="BAD DATA";
		}
		array_push($q,$rr);
	}
	$output['quotes']=$q;
...
// Members loaded in INVOICE LOAD
	...
	function getCompanyDashboard($data) {
		...
		$r=array();
                $total_terms=0;
		foreach($e as $f) {
			   //-- 5/27: Active plans have blank end_month_id.
                           $sql="select * from nua_company_plan where company_id = " . $data['id'] . " and ";
			   $sql.="APA_CODE = '" . $f['plan'] . "' and end_month_id = ''";
                           $p=$this->X->sql($sql);
                           if (sizeof($p)>0) {
...
$output['total_revenue']=$total_revenue;
		...
		// 5/27: Active plans have blank end_month_id.
                $sql="select * from nua_company_plan where company_id = " . $data['id'] . " and end_month_id = '' order by plan_name";
		$e=$this->X->sql($sql);
                $output['plans']=$e;
