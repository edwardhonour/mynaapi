...
function postAddLevel($data) {
    ...
    function getUser($data) {
		...
		if (!isset($data['uid'])) {
			...
			$sql="select id, role, org_id, company_id, employee_id, last_login, last_timestamp, email from nua_user where id = " . $data['uid'];
...
function postAnswer($data) {
   ...
   $post=array();
   $post['table_name']="sqllabs_flashcard_answers";
   $post['action']="insert";
   $post['user_id']=$data['data']['user']['id'];
   $post['session_id']=$data['data']['formData']['session_id'];
   $post['section']=$data['data']['formData']['section'];
   $post['question_id']=$data['data']['formData']['question_id'];
   $post['answer']=$data['data']['formData']['answer'];

   $sql="select * from sqllabs_flashcards where id = " . $data['data']['formData']['question_id'];
   $t=$this->X->sql($sql);
   if ($t[0]['answer']==$data['data']['formData']['answer']) {
	   $post['correct']='Y';
   } else {
           $post['correct']='N';
   }

   $correct_answer=$t[0]['answer'];
   $post['correct_answer']=$t[0]['answer'];
   $post['level']=$t[0]['level'];
   $post['answer']=$data['data']['formData']['answer'];
   $this->X->post($post);

   $sql="select * from sqllabs_flashcard_sessions where id = " . $post['session_id'];
   $h=$this->X->sql($sql);

   $correct=$h[0]['correct'];
   $questions=$h[0]['questions'];
   $questions++;
   if ($post['correct']=='Y') $correct++;

   $sql="update sqllabs_flashcard_sessions set correct = " . $correct . ", questions = " . $questions;
   $sql.=" where id = " . $post['session_id'];

   $this->X->execute($sql);

   $formData=array();
   $formData=$data['data']['formData'];
   if ($post['correct']=='Y') $formData['answer']="";
 $formData['correct']=$correct;
 $formData['total']=$questions;
 $pct=round(($correct/$questions)*100,2);
	 $formData['percent']=$pct;
  	 if ($post['correct']=='Y') { $formData['is_correct']='Y'; } else { $formData['is_correct']='N'; }
  	 if ($post['correct']=='N') { $formData['is_incorrect']='Y'; } else { $formData['is_incorrect']='N'; }
	 $formData['correct_answer']=$correct_answer;

	 if ($post['correct']=='Y') {
		 $out=$this->getNextQuestion($data);
		 $formData['question_id']=$out['question_id'];
		 $formData['question']=$out['question'];
	 }
	 return $formData;
...
function getNextQuestion($data) {
	...
	$post=array();
	$post['section']=$data['data']['formData']['section'];
	$post['level']=$data['data']['formData']['level1'];
	$post['level2']=$data['data']['formData']['level2'];
	$post['level3']=$data['data']['formData']['level3'];
	$post['level4']=$data['data']['formData']['level4'];
	$post['level5']=$data['data']['formData']['level5'];
	$post['user_id']=$data['data']['user']['id'];

        $sql="select count(*) as c from sqllabs_flashcards where section = '" . $post['section'] . "' ";
	$sql.=" and (id = 0 ";
	if ($post['level']==1) $sql.= " or level = 1 ";
	if ($post['level2']==1) $sql.= " or level = 2 ";
	if ($post['level3']==1) $sql.= " or level = 3 ";
	if ($post['level4']==1) $sql.= " or level = 4 ";
	if ($post['level5']==1) $sql.= " or level = 5 ";
	$sql.=")";
	...
	$val=rand(1,$count);
        ...
        $sql="select * from sqllabs_flashcards where section = '" . $post['section'] . "' ";
	$sql.=" and (id = 0 ";
	if ($post['level']==1) $sql.= " or level = 1 ";
	if ($post['level2']==1) $sql.= " or level = 2 ";
	if ($post['level3']==1) $sql.= " or level = 3 ";
...
function getNext($data) {
    ...
    $formData['question_id']=$out['question_id'];
    $formData['question']=$out['question'];
    $formData['is_incorrect']='N';
    $formData['is_correct']='N';
    $formData['answer']="";
...
function startSession($data) {
	$post=array();
	$post['table_name']="sqllabs_flashcard_sessions";
	$post['action']="insert";
	$post['section']=$data['data']['formData']['section'];
	$post['level']=$data['data']['formData']['level1'];
	$post['level2']=$data['data']['formData']['level2'];
	...
	$post['level5']=$data['data']['formData']['level5'];
	$post['user_id']=$data['data']['user']['id'];
	$session_id=$this->X->post($post);
	$data['data']['formData']['session_id']=$session_id;
	$out=$this->getNextQuestion($data);

	 $formData=array();
	 $formData['section']=$post['section'];
	 $formData['session_id']=$session_id;
	 $formData['answer']="";
	 $formData['question_id']=$out['question_id'];
	 $formData['question']=$out['question'];
	 if ($post['level']==1) { $formData['level1']=true; } else { $formData['level1']=false; }
	 if ($post['level2']==1) { $formData['level2']=true; } else { $formData['level2']=false; }
	 if ($post['level3']==1) { $formData['level3']=true; } else { $formData['level3']=false; }
	 if ($post['level4']==1) { $formData['level4']=true; } else { $formData['level4']=false; }
	 if ($post['level5']==1) { $formData['level5']=true; } else { $formData['level5']=false; }
	 $formData['correct']=0;
	 $formData['total']=0;
	 $formData['percent']=0;
  	 $formData['is_correct']='N';
	...
	function getCardPage($data) {
		 $output=$this->start_output($data);
		 if ($output['user']['forced_off']>0) return $output;
		 $formData=array();
		 $formData['section']=$data['id'];
		 $formData['session_id']=0;
		 $formData['answer']="";
		 $formData['question_id']="";
		 $formData['question']="Start a session to begin.";
		 $formData['level1']=true;
		 $formData['level2']=true;
		 $formData['level3']=true;
		 $formData['level4']=true;
		 $formData['level5']=true;
		 $formData['correct']=0;
		 $formData['total']=0;
		 $formData['percent']=0;
		 $formData['is_correct']='N';
...
function getTestDashboard($data) {
	...
	$plsql_correct=0;
	$typescript_count=0;
	$typescript_correct=0;
	$rxjs_count=0;
	$rxjs_correct=0;


	$sql="select id, create_timestamp, section, questions, correct from sqllabs_flashcard_sessions where user_id = " . $uid;
	$sql.=" and questions > 0 order by id desc";
	$t=$this->X->sql($sql);
	foreach($t as $u) {
		if ($u['section']=='HTML') {
			$html_count+=$u['questions'];
			$html_correct+=$u['correct'];
		}
		if ($u['section']=='CSS') {
			$css_count+=$u['questions'];
			$css_correct+=$u['correct'];
		}
	}

	$output['html_count']=$html_count;
	$output['html_correct']=$html_correct;
	$p=round(($html_correct/$html_count)*100,2);
	$output['html_pct']=$p;
	$output['css_count']=$css_count;
	$output['css_correct']=$css_correct;
	$output['direct_count']=$direct_count;
	$output['direct_correct']=$direct_correct;
	$output['event_count']=$event_count;
	$output['event_correct']=$event_correct;
	$output['bootstrap_count']=$bootstrap_count;
	$output['bootstrap_correct']=$bootstrap_correct;
	$output['plsql_count']=$bootstrap_count;
	$output['plsql_correct']=$bootstrap_correct;
	$output['typescript_count']=$bootstrap_count;
	$output['typescript_correct']=$bootstrap_correct;
	$output['rxjs_count']=$rxjs_count;
	$output['rxjs_correct']=$rxjs_correct;
	...
	function getEmployeeDashboard($data) {
		...
		if (sizeof($d)>0) {
			...
			$output['formData']=$d[0];
                        ...
                        $sql="select * from nua_census where employee_id = " . $data['id'];
                        $j=$this->X->sql($sql);
                        if (sizeof($j)>0) {
                             $census=$j[0];
                        } else {
                             ...
                             $census['copmany_name']="";
                             $census['company_id']=0;
                             $census['alt_company_name']="";
                             $census['alt_company_id']=0;
                        }
...
print_r($customHeaders);
	...
	function getCompanyList($data) {
		...
		$user=$output['user'];
	        $output['id']=$data['id'];	
		$sql="select * from nua_company where 1 = 0 order by company_name";
		if ($user['role']=="sadmin"||$user['role']=='user') {
                      if ($data['id']=="active") {
			$sql="select * from nua_company where status in ('enrolled','terminated')  order by company_name";
                     } else {
if ($data['id']=="apa") {
			$sql="select * from nua_company where id in (select distinct company_id from nua_census)  order by company_name";
} else {
			$sql="select * from nua_company order by company_name";
}
		     }
		}
		if ($user['role']=="badmin") {
		...
		if ($user['role']=="broker") {
			$sql="select * from nua_company where created_id = " . $data['uid'] . " order by company_name";
		}		
