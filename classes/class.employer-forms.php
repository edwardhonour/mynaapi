...
function getHrHome($data) {
	...
	$sql="select * from nua_employee where company_id = " . $company_id . " order by last_name, first_name";
        $r=$this->X->sql($sql);
	$output['census']=$r;
	...
	function getInvoiceList($data) {
		...
		$sql="select * from nua_company where id = " . $company_id . " order by company_name";
                ...
                foreach($d as $e) {
                    $h=array();
                    $h['company_id']=$e['id'];
                    $h['company_name']=$e['company_name'];
                    $h['id']=$e['id'];
                    $h['billing_contact_name']=$e['billing_contact_name'];
		    ...
		    $h['invoices']=$g; 
                    $total=0;
                    foreach($g as $t) { $total+=$t['grand_total_float']; $grand_total+=$t['grand_total_float']; } 
                    $h['company_total']=number_format($total,2);
                    array_push($list,$h);
                }
		$output['list']=$list;
                $output['grand_total']=number_format($grand_total,2);
		...
		$l['date']="03/15/2022";
		$l['invoice']="5556-2203";
		$l['type']="Invoice/Benefits";
		$l['description']="Member Medical/Dental/Vision Invoice - March 2022";
		$l['payments']="0.00";
		$l['invoices']="6,968.31";
		$l['balance']="-6,968.31";
                array_push($list,$l);
		$output['list']=$list;
		$output['payments']="0.00";
		$output['invoices']="6,968.31";
		$output['balance']="-6,969.31";
